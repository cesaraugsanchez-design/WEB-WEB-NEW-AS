...
export function opcionesDeFiltro(registros) {
  ...
  for (const { campo } of DIMENSIONES) {
    ...
    const lista = [...conteo].map(([valor, n]) => ({ valor, n }))
    if (campo === 'anio') lista.sort((a, b) => b.valor.localeCompare(a.valor))
    else lista.sort((a, b) => b.n - a.n)
    salida[campo] = lista
  }
  return salida
}
...
function agrupar(registros, campo) {
  ...
  for (const r of registros) {
    ...
    if (!m.has(k)) m.set(k, { etiqueta: k, total: 0, cerrados: 0, abiertos: 0 })
    const g = m.get(k)
    g.total += 1
...
export function calcularMetricas(registros, { anio = null, hoy = null } = {}) {
  ...
  const abiertos = registros.filter(esAbierto).length
