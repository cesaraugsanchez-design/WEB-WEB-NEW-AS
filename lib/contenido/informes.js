/**
 * La Semanal — informe de mercado asegurador de ASSANCH.
 *
 * EL PDF NO SE PUBLICA. Solo se publican imagenes de sus paginas, para que
 * quien entre pueda leer el informe sin obtener el archivo. Esconder un boton
 * de descarga no protege nada: la URL del PDF seguiria siendo accesible.
 *
 * Eso NO impide una captura de pantalla, y ninguna tecnologia lo impide. Lo que
 * consigue es que nadie se lleve el documento original.
 *
 * ANTES DE PUBLICAR: se elimina el apartado «Implicaciones para Assanch»
 * —lectura estrategica interna de la firma— para que el informe se pueda
 * compartir con personal externo. Se hace al convertir, con el cuarto argumento
 * del script, no despues: si la pagina llega a public/ ya es publica.
 *
 * COMO ANADIR UN INFORME:
 *   1. Localizar la pagina del apartado interno:
 *        swift herramientas/buscar-apartado.swift informe.pdf "Implicaciones para Assanch"
 *      Aparece dos veces: en el indice (pag. 2) y en la seccion. Solo se excluye
 *      la seccion. Si continua en la pagina siguiente, se excluyen las dos.
 *   2. Convertir excluyendola:
 *        swift herramientas/pdf-a-imagenes.swift informe.pdf public/informes/AAAA-MM-DD 2.0 7
 *   3. Anadir una entrada AL PRINCIPIO de este array, con el numero de paginas
 *      RESULTANTE, no el del PDF original.
 *   4. NO copiar el PDF a public/. Guardarlo fuera del repositorio.
 *
 * `resumen` NO es un adorno de la tarjeta: el informe se publica como imagenes,
 * ilegible para un lector de pantalla, y ese texto es la unica via de acceso
 * para quien no ve. Tambien es lo unico que puede indexar un buscador.
 * Los de aqui son el resumen ejecutivo real de cada informe, resumido.
 */

const paginas = (fecha, n) =>
  Array.from({ length: n }, (_, i) => `/informes/${fecha}/${String(i + 1).padStart(2, '0')}.png`)

export const informes = [
  {
    slug: '2026-09-21',
    semana: 39,
    titulo: 'Cuatro mil autorizaciones falsas y un colateral que nunca existió',
    fecha: '2026-09-21',
    resumen:
      'Dos de los tres frentes de la semana comparten un mismo hecho: el papel que sostenía la operación era falso. El caso Vesttoo produjo su primera condena penal, mientras Swiss Re Institute puso número a la cola que el mercado no termina de medir: un huracán categoría 5 sobre Miami o Tampa podría costar más de US$300,000 millones asegurados. En República Dominicana, la Pepca depositó la fase 2.0-A del caso Senasa: 4,363 autorizaciones médicas que, según la acusación, nunca correspondieron a un servicio prestado. Y el costo de asegurar mercancías transportadas cayó 17.5 % interanual en la región, sin que el riesgo de fondo se haya movido.',
    paginas: paginas('2026-09-21', 7),
  },
  {
    slug: '2026-09-11',
    semana: 37,
    titulo: 'El agua que no llega, la tierra que se movió y un atacante que no era humano',
    fecha: '2026-09-11',
    resumen:
      'En Montecarlo, el 68º Rendez-Vous cerró con la idea de que el precio de propiedad ya tocó suelo y que lo que seguirá cediendo es el clausulado. Colombia llegó al mes del terremoto de magnitud 7.4 con 331 fallecidos y más de 238,000 viviendas afectadas. En República Dominicana no llega el huracán, pero tampoco la lluvia: las presas están al 59 % y el déficit cae sobre los meses que aportan un tercio del agua del año. Cierra un anexo de ciberriesgo con el caso Hugging Face, la primera intrusión documentada ejecutada por un enjambre de agentes de IA.',
    paginas: paginas('2026-09-11', 8),
  },
  {
    slug: '2026-09-04',
    semana: 36,
    titulo: '800,000 millones de dólares: el reaseguro llega a Montecarlo con más capital que nunca',
    fecha: '2026-09-04',
    resumen:
      'En Montecarlo, Aon situó el capital global de reaseguro en un récord de 800,000 millones de dólares de cara al 1 de enero, mientras Verisk elevaba a 171,000 millones anuales la pérdida catastrófica que la industria debería estar preparada para absorber. En República Dominicana, Humano Seguros puso cifra al costo del ramo de vehículos: RD$100 millones mensuales en indemnizaciones y una siniestralidad de 55 a 56 %. La edición estrena dos apartados: el ranking del sector asegurador dominicano y el balance de medio año de la firma.',
    paginas: paginas('2026-09-04', 11),
  },
  {
    slug: '2026-08-28',
    semana: 35,
    titulo: 'El Súper Niño: el océano más caliente desde que hay registros',
    fecha: '2026-08-28',
    resumen:
      'Edición monográfica. La NOAA da más de nueve probabilidades sobre diez de que El Niño alcance categoría muy fuerte entre otoño e invierno. Para República Dominicana la traducción es sencilla: menos huracán y más falta de agua. Para el ajuste, cambia el tipo de siniestro —del daño por viento al daño por sequía, calor y paralización.',
    paginas: paginas('2026-08-28', 7),
  },
  {
    slug: '2026-08-21',
    semana: 34,
    titulo: '3.8 millones de motocicletas, siete de cada diez víctimas: la brecha dominicana que termina en el ajuste',
    fecha: '2026-08-21',
    resumen:
      'El mercado blando dejó de ser previsión y pasó a ser gestión: AM Best confirma apetito por catástrofe pese a caídas de precio de hasta 5.5 % en las renovaciones de julio, y la consolidación se acelera. En República Dominicana, Cadoar pone número a la brecha de protección: 3,846,694 motocicletas —el 57.9 % del parque— frente a una penetración del seguro del 1.9 % del PIB.',
    paginas: paginas('2026-08-21', 7),
  },
  {
    slug: '2026-08-14',
    semana: 33,
    titulo: 'Colombia: 2.2 millones de pólizas de terremoto para un sismo que no se puede tarifar todavía',
    fecha: '2026-08-14',
    resumen:
      'El sismo de magnitud 7,4 del 10 de agosto en San José del Palmar deja a la industria con un dato inédito para la región y una incógnita abierta: ningún modelador ni bróker ha publicado todavía una estimación formal de pérdida asegurada. En el frente global, el primer semestre desmiente la narrativa del año tranquilo. En República Dominicana, la siniestralidad de vehículos escaló a 65.42 % al cierre de mayo.',
    paginas: paginas('2026-08-14', 7),
  },
  {
    slug: '2026-08-07',
    semana: 32,
    titulo: 'La factura venezolana: cuando el reaseguro sobra y la cobertura falta',
    fecha: '2026-08-07',
    resumen:
      'El doblete sísmico de Venezuela se consolida como la mayor lección de la región en una década: pérdidas totales cercanas a US$30,000 millones con menos de US$1,000 millones asegurados. Mientras tanto, el mercado global vive la paradoja contraria: capital sobrante y bonos catastróficos en máximos históricos. En República Dominicana, la Resolución 01-2024 entra en fase final de implementación.',
    paginas: paginas('2026-08-07', 7),
  },
]

export const hayInformes = informes.length > 0

/* Del mas reciente al mas antiguo, sin depender de que se hayan escrito en
   orden en el array. */
export const informesOrdenados = [...informes].sort((a, b) => b.fecha.localeCompare(a.fecha))

export const informePorSlug = Object.fromEntries(informes.map((i) => [i.slug, i]))

/* La portada es siempre la primera pagina. */
export const portadaDe = (informe) => informe.paginas[0]
