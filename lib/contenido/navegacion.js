...
import { hayInformes, informes } from './informes'
    ...
    columnas: [
      {
        titulo: 'Qué hacemos',
        enlaces: [
          ...servicios.map((s) => ({ label: s.titulo, href: `/servicios/${s.slug}` })),
          { label: 'Metodología y tecnología', href: '/tecnologia' },
        ],
      },
    ],
  },
    ...
    columnas: [
      {
        titulo: 'La Semanal',
        enlaces: [
          ...informes.slice(0, 3).map((i) => ({
            label: `Semana ${i.semana} · ${i.titulo.split(':')[0]}`,
            href: `/mercado-asegurador/${i.slug}`,
          })),
        ],
        pie: { label: 'Ver todos los informes', href: '/mercado-asegurador' },
      },
      {
        titulo: 'Guías',
        enlaces: recursos.slice(0, 4).map((r) => ({ label: r.titulo, href: `/recursos/${r.slug}` })),
        pie: { label: 'Ver todos los recursos', href: '/recursos' },
      },
    ],
  },
  {
...
export const rutasEstaticas = [
  ...
  '/experiencia',
  '/recursos',
  ...recursos.map((r) => `/recursos/${r.slug}`),
  ...(hayInformes
    ? ['/mercado-asegurador', ...informes.map((i) => `/mercado-asegurador/${i.slug}`)]
    : []),
  '/cobertura',
  '/someter-reclamo',
]
